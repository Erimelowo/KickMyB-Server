...
class ServiceTaskTests {
    ...
    @Test
    void testSupprimerTacheOk() throws ServiceTask.Empty, ServiceTask.TooShort, ServiceTask.Existing,
            ServiceAccount.UsernameTooShort, ServiceAccount.PasswordTooShort,
            ServiceAccount.UsernameAlreadyTaken, BadCredentialsException, ServiceTask.NotOwner, ServiceTask.TaskNotFound {
        ...
        // on re-récupère l'utilisateur
        alice = serviceTask.userFromUsername("alice");

        // on supprime la tâche
        serviceTask.delete(serviceTask.home(alice.id).getFirst().id, alice);

        // on vérifie que la tâche a bien été supprimée
        assertEquals(0, serviceTask.home(alice.id).size());
    }

    @Test
    void testSupprimerTacheMauvaisIdKo() throws ServiceTask.Empty, ServiceTask.TooShort, ServiceTask.Existing,
            ServiceAccount.UsernameTooShort, ServiceAccount.PasswordTooShort,
            ServiceAccount.UsernameAlreadyTaken, BadCredentialsException, ServiceTask.NotOwner, ServiceTask.TaskNotFound {
        ...
        try {
            // on supprime la tâche
            serviceTask.delete(1234, alice); // Mauvais ID
        } catch (ServiceTask.TaskNotFound ignored) {
        }

        // on vérifie que la tâche existe encore
        assertEquals(1, serviceTask.home(alice.id).size());
    }

    @Test
    void testSupprimerTacheNotOwnerKo() throws ServiceTask.Empty, ServiceTask.TooShort, ServiceTask.Existing,
            ServiceAccount.UsernameTooShort, ServiceAccount.PasswordTooShort,
            ServiceAccount.UsernameAlreadyTaken, BadCredentialsException, ServiceTask.NotOwner, ServiceTask.TaskNotFound {
        ...
        req2.username = "bob";
        req2.password = "Passw0rd!";
        serviceAccount.signup(req2);
        ...
        try {
            // on supprime la tâche
            serviceTask.delete(serviceTask.home(alice.id).getFirst().id, bob); // Mauvais owner
        } catch (ServiceTask.NotOwner ignored) {
        }
